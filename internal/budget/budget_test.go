...
func TestProfiles_KnownNames(t *testing.T) {
	require.Contains(t, budget.Profiles, "quick")
	require.Contains(t, budget.Profiles, "default")
	require.Contains(t, budget.Profiles, "deep")
	require.Equal(t, 60, budget.Profiles["default"].MaxIter)
	require.Equal(t, 20, budget.Profiles["default"].MaxCents)
	require.Equal(t, 1800, budget.Profiles["default"].MaxWallSec)
}

func TestParseBudgetFlag_NoFlag(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("build something")
	require.Equal(t, "default", profile)
	require.Equal(t, "build something", desc)
}

func TestParseBudgetFlag_DeepFlag(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("--budget=deep build a complex thing")
	require.Equal(t, "deep", profile)
	require.Equal(t, "build a complex thing", desc)
}

func TestParseBudgetFlag_QuickFlag(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("--budget=quick foo")
	require.Equal(t, "quick", profile)
	require.Equal(t, "foo", desc)
}

func TestParseBudgetFlag_UnknownProfile(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("--budget=hyperultra do thing")
	require.Equal(t, "default", profile)
...
func TestParseBudgetFlag_NoSpaceAfter(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("--budget=deep")
	require.Equal(t, "default", profile)
...
func TestParseBudgetFlag_LeadingWhitespace(t *testing.T) {
	profile, desc := budget.ParseBudgetFlag("   --budget=deep do thing")
	require.Equal(t, "deep", profile)
...
func TestParseBudgetFlag_FlagInMiddle_NotMatched(t *testing.T) {
	...
	profile, desc := budget.ParseBudgetFlag("build a thing --budget=deep should be in desc")
	require.Equal(t, "default", profile)
